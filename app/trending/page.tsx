...
type Post = {
  _id: string; // Add _id for consistency with the API response
  user: string;
  content: string;
  ratings: number; // Use number instead of Number (primitive type)
};

export default function TrendyPage() {
  const [posts, setPosts] = useState<Post[]>([]); // State initialized with empty array of Post type
    ...
    <div className="mt-16 flex flex-col lg:flex-row">
      ...
      <div className="p-4 w-full lg:w-1/6 bg-headerBlue lg:bg-gray-50 text-white fixed bottom-0 lg:sticky lg:top-0 lg:h-screen flex flex-col items-center">
        ...
        <div className="mb-4 hidden lg:flex flex-col items-center space-y-2">
          ...
          <div className="text-center">
            <p className="text-lg font-bold text-white lg:text-gray-900">
              Code with Rensy
            </p>
      ...
      <div className="w-full lg:w-5/6 bg-gray-50 p-10 h-screen flex flex-col items-center">
        ...
        <div className="space-y-6 mt-10 lg:w-1/2">
          {/* Post */}
          {posts.map((post) => (
            <div className="p-4 border rounded-lg shadow-sm bg-white" key={post._id}>
            {/* Post Header */}
